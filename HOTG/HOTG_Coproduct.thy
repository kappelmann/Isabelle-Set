...
(* definition "coprod A B \<equiv> has_inverse_on A inl \<squnion> has_inverse_on B inr" *)
...
lemma mem_coprod_iff [iff]:
  "x \<in> A \<Coprod> B \<longleftrightarrow> (\<exists>a : A. x = inl a) \<or> (\<exists>b : B. x = inr b)"
...
lemma mono_subset_coprod: "((\<subseteq>) \<Rightarrow> (\<subseteq>) \<Rrightarrow> (\<subseteq>)) (\<Coprod>)" by blast
