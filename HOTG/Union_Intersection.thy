...
lemma interD [dest]: "\<lbrakk>A \<in> \<Inter>C; B \<in> C\<rbrakk> \<Longrightarrow> A \<in> B"
  by auto

lemma union_empty_eq [simp, intro!]: "\<Union>{} = {}" by auto

lemma inter_empty_eq [simp, intro!]: "\<Inter>{} = {}"
...
proof
  ...
  proof (rule or_if_not_imp)
    ...
    show "A = {{}}" by (rule eqI) auto
  qed
qed auto

lemma union_eq_empty_iff': "\<Union>A = {} \<longleftrightarrow> (\<forall>B \<in> A. B = {})"
  by blast

lemma union_singleton_eq [simp, intro!]: "\<Union>{b} = b"
  by (rule eq_if_subset_if_subset) auto

lemma inter_singleton_eq [simp, intro!]: "\<Inter>{b} = b"
  by (rule eq_if_subset_if_subset) auto

lemma subset_union_if_mem: "B \<in> A \<Longrightarrow> B \<subseteq> \<Union>A"
  by blast

lemma inter_subset_if_mem: "B \<in> A \<Longrightarrow> \<Inter>A \<subseteq> B"
  unfolding inter_def by blast
...
lemma union_subset_if_all_mem_subset: "(\<And>x. x \<in> A \<Longrightarrow> x \<subseteq> C) \<Longrightarrow> \<Union>A \<subseteq> C"
  unfolding subset_def by blast
...
lemma mem_idx_union_iff: "b \<in> (\<Union>x \<in> A. (B x)) \<longleftrightarrow> (\<exists>x \<in> A. b \<in> B x)"
  by auto

lemma mem_idx_inter_iff: "b \<in> (\<Inter>x \<in> A. (B x)) \<longleftrightarrow> A \<noteq> {} \<and> (\<forall>x \<in> A. b \<in> B x)"
  by auto

(*LP: The order of the premises presupposes that A is rigid; b may be flexible*)
lemma mem_idx_unionI [intro]: "a \<in> A \<Longrightarrow>  b \<in> B a \<Longrightarrow> b \<in> (\<Union>x \<in> A. B x)"
  by auto

lemma mem_idx_interI [intro]:
  "A \<noteq> {} \<Longrightarrow> (\<And>x. x \<in> A \<Longrightarrow> b \<in> B x) \<Longrightarrow> b \<in> (\<Inter>x \<in> A. B x)"
  by auto
...
lemma idx_union_empty_dom_eq [simp, intro!]: "(\<Union>x \<in> {}. B x) = {}"
  by auto

lemma idx_inter_empty_dom_eq [simp, intro!]: "(\<Inter>x \<in> {}. B x) = {}"
  unfolding inter_def by auto

lemma idx_union_empty_eq [simp, intro!]: "(\<Union>x \<in> A. {}) = {}"
  by auto

lemma idx_inter_empty_eq [simp, intro!]: "(\<Inter>x \<in> A. {}) = {}"
  unfolding inter_def by auto

lemma idx_union_eq_union [simp, intro!]: "(\<Union>x \<in> A. x) = \<Union>A"
  by (rule eqI) auto

lemma idx_inter_eq_inter [simp, intro!]: "(\<Inter>x \<in> A. x) = \<Inter>A"
  by (rule eqI) auto

lemma idx_union_subset_iff: "(\<Union>x \<in> A. B x) \<subseteq> C \<longleftrightarrow> (\<forall>x \<in> A. B x \<subseteq> C)"
  unfolding subset_def by blast
...
lemma subset_idx_union_if_mem: "x \<in> A \<Longrightarrow> B x \<subseteq> (\<Union>x \<in> A. B x)"
  by blast

lemma idx_inter_subset_if_mem: "x \<in> A \<Longrightarrow> (\<Inter>x \<in> A. B x) \<subseteq> B x"
  by blast
...
lemma idx_union_singleton_eq [simp, intro!]: "(\<Union>x \<in> A. {x}) = A"
  by (rule eq_if_subset_if_subset) auto
...
lemma idx_union_repl [simp]:
  "(\<Union>y \<in> {f x | x \<in> A}. B y) = (\<Union>x \<in> A. B (f x))"
  by (rule eq_if_subset_if_subset) auto

lemma idx_inter_repl [simp]:
  "(\<Inter>x \<in> {f x | x \<in> A}. B x) = (\<Inter>a \<in> A. B(f a))"
  unfolding inter_def by auto

lemma idx_inter_union_eq_idx_inter_idx_inter:
  "{} \<notin> A \<Longrightarrow> (\<Inter>x \<in> \<Union>A. B x) = (\<Inter>y \<in> A. \<Inter>x \<in> y. B x)"
  by (rule eqI) (auto simp: union_eq_empty_iff)
...
next
  ...
  with \<open>y \<in> A\<close> have "{B x | x \<in> A} \<noteq> {{}}" by (auto dest: eqD)
  with x_mem show "x \<in> (\<Inter>z \<in> (\<Union>x \<in> A. B x). C z)" by (auto simp: union_eq_empty_iff)
qed
...
lemma empty_bin_union_eq [simp, intro!]: "{} \<union> A = A"
  by (rule eq_if_subset_if_subset) auto

lemma bin_union_empty_eq [simp, intro!]: "A \<union> {} = A"
  by (rule eq_if_subset_if_subset) auto
...
lemma mem_bin_union_singleton [simp, intro!]: "b \<in> A \<union> {b}" by auto
...
lemma subset_iff_bin_union_eq_right: "A \<subseteq> B \<longleftrightarrow> A \<union> B = B"
  unfolding subset_def by (auto intro: eqI' dest: eqD)

lemma subset_iff_bin_union_eq_left: "A \<subseteq> B \<longleftrightarrow> B \<union> A = B"
  unfolding subset_def by (auto intro: eqI' dest: eqD)

lemma bin_union_eq_empty_iff [iff]: "(A \<union> B = {}) \<longleftrightarrow> (A = {} \<and> B = {})"
  by blast

text\<open>Binary intersection\<close>
...
lemma empty_bin_inter_eq_empty [simp, intro!]: "{} \<inter> B = {}"
  by auto

lemma bin_inter_empty_eq_empty [simp, intro!]: "A \<inter> {} = {}"
  by auto

lemma bin_inter_subset_iff [iff]: "C \<subseteq> A \<inter> B \<longleftrightarrow> C \<subseteq> A \<and> C \<subseteq> B"
  unfolding subset_def by blast

lemma bin_inter_subset_left [simp, intro!]: "A \<inter> B \<subseteq> A"
  by blast

lemma bin_inter_subset_right [simp, intro!]: "A \<inter> B \<subseteq> B"
  by blast

lemma subset_bin_inter_if_subset_if_subset: "\<lbrakk>C \<subseteq> A; C \<subseteq> B\<rbrakk> \<Longrightarrow> C \<subseteq> A \<inter> B"
  by blast

lemma bin_inter_self_eq_self [simp, intro!]: "A \<inter> A = A"
  by (rule eq_if_subset_if_subset) auto

lemma bin_inter_absorb [simp, intro!]: "A \<inter> (A \<inter> B) = A \<inter> B"
  by (rule eq_if_subset_if_subset) auto
...
lemma bin_inter_collect_absorb [simp, intro!]:
...
proof
  ...
  proof (rule subsetI)
    ...
    with mem_idx_union_iff obtain x where "x \<in> I" and "a \<in> A \<inter> B x" by auto
    then show "a \<in> ?lhs_union" by auto
  qed
qed (auto 5 0 intro!: eqI simp: subset_def)
