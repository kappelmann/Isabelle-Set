...
lemma Int_Rep_add_type [type]: "Int_Rep_add \<Ztypecolon> Int_Rep \<Rightarrow> Int_Rep \<Rightarrow> Int_Rep"
proof (intro Dep_funI)
(*TODO: this should be automatically provable by the type-checker*)
  fix x y assume [type]: "x \<Ztypecolon> Int_Rep" "y \<Ztypecolon> Int_Rep"
  have "(\<lambda>m. nat_rec m y Int_Rep_succ) \<Ztypecolon> Nat \<Rightarrow> Int_Rep"
    by discharge_types
  moreover have "(\<lambda>m. nat_rec m y Int_Rep_pred) \<Ztypecolon> Element (\<nat> \<setminus> {0}) \<Rightarrow> Int_Rep"
  proof -
    {
      fix m assume "m \<Ztypecolon> Element (\<nat> \<setminus> {0})"
      then have "m \<Ztypecolon> Nat" by (auto intro: ElementI dest: ElementD)
    }
    then show ?thesis by discharge_types
  qed
  ultimately show "Int_Rep_add x y \<Ztypecolon> Int_Rep"
    unfolding Int_Rep_add_def Int_Rep_def by discharge_types
qed

lemma Int_Rep_succ_add_eq_succ_add:
  assumes "x \<Ztypecolon> Int_Rep" "y \<Ztypecolon> Int_Rep"
  shows "Int_Rep_add (Int_Rep_succ x) y = Int_Rep_succ (Int_Rep_add x y)"
using \<open>x \<Ztypecolon> Int_Rep\<close>
...
proof (cases x rule: Int_RepE)
  (*we first prove a generalised version for the nonnegative case*)
  {
    ...
    from \<open>n \<in> \<nat>\<close> have "y \<Ztypecolon> Int_Rep \<Longrightarrow>
    ...
    next
      ...
      next
        ...
        proof -
          ...
          have "Int_Rep_add (Int_Rep_neg n) (Int_Rep_inv y) \<Ztypecolon> Int_Rep"
          proof -
            from \<open>n = succ _\<close> have "n \<Ztypecolon> Element (\<nat> \<setminus> {0})" by auto
            then show ?thesis by discharge_types
          qed
...
proof (cases x rule: Int_RepE)
  {
    fix n y assume "n \<in> \<nat>" "y \<Ztypecolon> Int_Rep"
...
proof (cases x rule: Int_RepE)
  {
    fix n y z assume "n \<in> \<nat>" "y \<Ztypecolon> Int_Rep" "z \<Ztypecolon> Int_Rep"
...
lemma Int_Rep_sub_type [type]:
  "Int_Rep_sub \<Ztypecolon> Int_Rep \<Rightarrow> Int_Rep \<Rightarrow> Int_Rep"
