...
subsection \<open>Successor, Predecessor, and Inverse\<close>
theory TSIntegers_Rep_Succ_Pred_Inv
...
lemma Int_Rep_succ_type [type]: "Int_Rep_succ \<Ztypecolon> Int_Rep \<Rightarrow> Int_Rep"
proof -
  {
    fix n assume "n \<in> \<nat>" "n \<noteq> 0" "n \<noteq> 1"
    then have "pred n \<in> \<nat> \<setminus> {0}" using Nat_pred_eq_zero_iff by auto
    then have "Int_Rep_neg (pred n) \<Ztypecolon> Int_Rep" by discharge_types
  }
  note this[intro]
  show ?thesis
    unfolding Int_Rep_succ_def by (intro Dep_funI, elim Int_RepE) auto
qed
...
lemma Int_Rep_pred_type [type]:
  "Int_Rep_pred \<Ztypecolon> Int_Rep \<Rightarrow> Int_Rep"
  unfolding Int_Rep_pred_def by (intro Dep_funI, elim Int_RepE) auto
...
(* lemma
  assumes "n : Nat"
  shows "(if n = 0 then Int_Rep_nonneg n else Int_Rep_neg n) \<Ztypecolon> Int_Rep"
proof -
  {
    assume "n \<noteq> 0"
    then have "n \<Ztypecolon> Element (\<nat> \<setminus> {0})" by (auto intro: ElementI)
  }
...
lemma Int_Rep_inv_type [type]: "Int_Rep_inv \<Ztypecolon> Int_Rep \<Rightarrow> Int_Rep"
proof -
  {
  ...
  note this[dest]
  show ?thesis
    unfolding Int_Rep_inv_def by (intro Dep_funI, elim Int_RepE) auto
qed

lemma Int_Rep_inv_inv_eq [simp]:
  "x \<Ztypecolon> Int_Rep \<Longrightarrow> Int_Rep_inv (Int_Rep_inv x) = x"
