...
shows "P n"
  apply (rule def_lfp_induct[OF nat_op_monop NAT_def, unfolded nat_op_def])
...
shows "P n"
  using assms nat_induct by unfold_types auto

lemma nat_is_ord: "x : nat \<Longrightarrow> x : Ord"
  by (induct x rule: nat_induct_typed) (auto intro: succ_Ord)

lemma zero_type: "{} : nat" by unfold_types auto
lemma succ_type: "succ : nat \<Rightarrow> nat" by unfold_types auto
