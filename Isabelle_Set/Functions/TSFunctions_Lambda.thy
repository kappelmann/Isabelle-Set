...
theory TSFunctions_Lambda
  imports
    TSFunctions_Extend_Restrict
    HOTG.SFunctions_Lambda
begin

lemma lambda_type [type]:
  "lambda : (A : Set) \<Rightarrow> ((x : Element A) \<Rightarrow> B x) \<Rightarrow> (x : Element A) \<rightarrow>cs B x"
  by unfold_types (auto intro!: left_total_onI intro!: in_domI simp: meaning_of_type)
...
next
  fix p assume "p \<in> \<lambda>x \<in> A'. f`x"
  with assms obtain x where "p = \<langle>x, f`x\<rangle>" "x \<in> A'" "x : A" by auto
  moreover then have "\<langle>x, f`x\<rangle> \<in> f" by (subst Dep_Function_pair_mem_iff_eval_eq) auto
  ultimately show "p \<in> f\<restriction>\<^bsub>A'\<^esub>"
    by (urule mem_rel_restrict_leftI where chained = insert) (auto simp only: mem_of_eq)
qed
