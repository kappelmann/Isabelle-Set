...
lemma lambda_app_type [backward_derive]:
  ...
  shows "lambda A f : (x : Element A) \<rightarrow>c B x"
  by discharge_types
...
lemma Dep_Function_lambda_bin_inter_type:
  assumes "f : (x : Element A) \<rightarrow> B x"
  shows "(\<lambda>x \<in> A \<inter> A'. f`x) : (x : Element (A \<inter> A')) \<rightarrow>c B x"
proof -
  from assms have "f : (x : Element (A \<inter> A')) \<rightarrow> B x"
