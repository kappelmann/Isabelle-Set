...
\maketitle
\begin{abstract}
The transfer tool\cite{huffman2013lifting} implements good automation to translate statements between different representations and the lifting tool\cite{huffman2013lifting} helps users to lift terms from one Pure-type to another and provides rules about these terms for the transfer tool. However, due to technical limitations, when working with the soft type system, implemented within the Isabelle/Set framework\cite{isabelleset}, one can not use the lifting tool to lift terms between soft types.
