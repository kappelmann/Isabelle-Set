...
For instance, we can express the connection between \(0_nat\) and \(0_\inte\) as \(\mathsf{T}_{\nat, \inte}\ 0\ 0\) with a suitable transfer relation \(\mathsf{T}_{\nat, \inte}\).
...
Working with a static type system,
such as offered by Isabelle/Pure,
has many benefits:
every term has a unique type and types are automatically checked and inferred.
However, as mentioned above, when one wants to refine a Pure-type to some set of values, the operations and theorems of the original type are not applicable to new type and vise versa.
The soft type system offers a solution to this problem.
In this system, a type is represented by a predicate and operations and theorems can be reused between all soft types defined on a shared Pure-type.
...
% Since, from the perspective of the type system, these type are not related, operations defined on one type can not be used for the other type. Similarly, the sematic connection between the types \(\nat\), \(\inte\) and \(\rat\) can not be expressed in the HOL type system and hence theorems proven about \(\nat\) are not directly applicable to the set of non-negative integers.
...
This is especially useful when one takes the approach common in mathematics to encode a variety of algebraic structures, datatypes and even functions as sets, the Pure-type system is of little use to prevent errors otherwise detected by a type checker.

While in some scenarios, one can reduce the need for the Lifting tool by using soft types instead of Pure-types defined by the \(\mathtt{typedef}\) command, there are many applications of lifting between soft types. The Lifting tool is, however, designed to lift terms between Pure-types and hence it is not applicable to soft types.

In this paper, we investigate the limitations of the Lifting tool, introduce a generalization of the theory on which the Lifting tool is based and propose a new lifting algorithm.

\cref{sec-prelims} introduces soft types and the theoretical foundations of the Transfer and Lifting tool. In \cref{lift-trip-sec} we explain the limitations of the concept introduced in the previous section and generalize them appropriately. Based on these generalizations, we propose a new lifting algorithm in \cref{sec:lift-alg} and \cref{sec:appl} demonstrates the application of this algorithm in the a set theoretic framework. Finally \cref{sec:concl} summarizes our work and outlines further work.

\section{Preliminaries}\label{sec-prelims}

\paragraph{Isabelle}
Reference to isabelle manual

\paragraph{Soft types}
Soft types [ref to github] allow the user to encode arbitrary properties in a type. In this type system, a type is represented by a predicate over a Pure-type. If a term \(x\) has the soft type \(\tau\), we write \(x : \tau\). The most primitive way to construct a soft type is using the function \(\mathsf{type}\) which takes a predicate as input and return the soft type containing exactly the values satisfying that predicate. Formally, \(x : \mathsf{type}\ P\) iff \(P\ x\). For instance, the soft type \(\mathsf{type}\ (\lambda i :: \inte\ldotp 0 \leq i)\) encodes the natural numbers within the Pure-type \(\inte\). To further refine a given soft type \(T\) with a predicate \(P\), one can use the operator \((\adj)\) where \(x : P \adj T\) iff \(P\ x \land x : T\). Since one often wants do define a soft type which inhabits all elements of a given set, we use the notation \([A]\) as an abbreviation for \(\mathsf{type}\ (\lambda x\ldotp x \in A)\).
...
This can be used to select a suitable transfer relation given a Pure-type.
...
It, however, we deal with potentially non-right-total lifting relations, there may be many lifting relations with the same Pure-type.
Note that, theoretically, we could always choose a soft type for a lifting relation \(T\)
such that \(T\) is right-total with respect to that type,
namely \(T : \alpha \rightarrow \mathsf{type}\ (\incodom\ T) \rightarrow \bool\),
but \(\mathsf{type}\ (\incodom\ T)\) this is in general not the type one would like to use and specify for $y$.
...
Nevertheless, given suitable types for the source term and the abstraction and representation functions involved in lifting it, we can get the expected type of the lifted term by type checking.
However, we cannot direct the lifting algorithm by
the Pure-types of the representation and abstract term and to direct it by the respective soft types, additional information about which lifting relation relates which soft types is required.
Since we want to keep the lifting algorithm independent from any type system, we did not incorporate the inference from types to lifting relations in our algorithm. Instead our algorithm takes a lifting relation as input and one can implement a preceding phase which computes a lifting relation from the source and target types for some type system.
...
We have also seen that we can not directly apply functions defined on one
...
Loosly speaking, a set extension would ``carve out''
...
together with the \liftrels \(\mathsf{Rel_\inte}\) and \(\eq\ \univ_{\mathsf{bool}}\).
...
\end{enumerate}
The first goal requires us to prove that the lifting relation of the dependent part of \(\mathsf{T_{div}}\), namely \(\reland\ \relint\ (\lambda j'\ldotp \mathsf{divides_\inte}\ j\ j')\), and the corresponding abstraction and representation function do indeed form a lifting triple. The second goal states that \(\reland\ \relint\ (\lambda j'\ldotp \mathsf{divides_\inte}\ j\ j')\) does not depend on which representative \(j\) we choose from any equivalence class of the co-domain of \(\mathsf{Rel_\inte}\). Note that the variables \(i\) and \(i'\) do not appear in the conclusion of this statement and hence, we can reduce the premise to \(\eqabs\ \rel_\inte\ j\ j'\). Furthermore, the conclusion is implied by \(\mathsf{divides}\ j = \mathsf{divides}\ j'\). Thus, it suffices to prove
\begin{equation*}
	\forall j\ j' \ldotp \eqabs\ \rel_\inte\ j\ j' \longrightarrow \mathsf{divides}\ j = \mathsf{divides}\ j'.
\end{equation*}
It is easy to see that this is the case since the equivalence classes of \(\eqabs\ \rel_\inte\) are singleton sets.
\end{example}
