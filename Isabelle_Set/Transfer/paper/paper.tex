...
\maketitle
\begin{abstract}
The Transfer tool implements good automation to translate statements between different representations and the Lifting tool helps users to lift terms from one Pure-type to another and provides rules about these terms for the Transfer tool. However, due to technical limitations, when working with the soft type system, implemented within Isabelle/Set, one can not use the Lifting tool to lift terms between soft types.

We present a generalization of the approach to lifting implemented by the Lifting tool which, in particular, allows us lift terms between soft types. We verified the theoretic foundations of our approach in Isabelle/HOL and propose a new lifting algorithm.
\end{abstract}
...
this relations is not captured by the type system.
In particular, one can not apply functions operating on one type to values of the other type.
...
Such frameworks are typically inspired by work of Reynolds [ref]
\kevin{add Mitchell and Wadler here}
...
The transfer tool [ref] of Isabelle/HOL allows one to translate theorems about one type to theorems about another type.
...
In \cref{lift-trip-sec} we describe the limitations of the existsing lifting tool
...
\paragraph{Isabelle}
In Isabelle/Pure, the logical framework on which Isabelle/HOL \cite{wenzel2021isabelle} is build on, every term has a unique Pure-type. We state that a term \(x\) has the Pure-type \(\tau\) by \(x :: \tau\) and for the Pure-type of functions with domain \(\sigma\) and co-domain \(\tau\) we write \(\sigma \Rightarrow \tau\).
...
To further refine a given soft type \(T\) with a predicate \(P\),
...
A function \(f\) has the soft type \((x : \sigma) \rightarrow \tau\ x\) iff \(\forall x\ldotp x : \sigma \longrightarrow f\ x : \tau\ x\).
...
The second goal states that \(\reland\ \relint\ (\lambda j'\ldotp \mathsf{divides_\inte}\ j\ j')\) does not depend on which representative \(j\) we choose from any equivalence class of the co-domain of \(\mathsf{Rel_\inte}\). Note that the variables \(i\) and \(i'\) do not appear in the conclusion of this statement and hence, we can reduce the premise to \(\eqabs\ \rel_\inte\ j\ j'\). Furthermore, the conclusion is implied by \(\mathsf{divides}\ j = \mathsf{divides}\ j'\). Thus, the second goal can be simplified to
\begin{equation*}
