...
goes via the intermediate level \(\rat\ \mathsf{set}\).
\kevin{in the next sentence and equation, all $\mathsf{fset}$ subscripts should be $\mathsf{set}$, right?}
For this, we need the function \(\mathsf{map_{fset}}\ f\ A \coloneqq \{f\ x \mid x \in A\}\).
Then we can prove
\begin{equation*}
...
As suggested by the blue dotted circles on the left side of \cref{fig:trans-rels-quot,fig:trans-rels-par-quot}, ``being related to the same abstract value'' is a PER for transfer relations form total and partial quotient types. For type copies and subtypes, at most one representation value is related to any abstract value and thus ``being related to the same abstract value'' is a restricted quality relations for these cases, which is PER as well.
...
be explained by the duality of subtypes and quotient types described by [source].
