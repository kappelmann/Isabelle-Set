...
In Isabelle/HOL there are two widely used commands to construct new types from existing ones: \(\mathtt{newtype}\) and \(\mathtt{quotient_type}\). The former defines a new type from a subset and the latter from a quotient set of some type. For instance, we can define the type \(\inte\) as a quotient of \(\nat \times \nat\) and the type of finite sets \(\alpha\ \mathsf{fset}\) as a subtype of \(\alpha\ \mathsf{set}\). Even though one type is defined as a subtype or quotient type of the other, this relations is not captured by the type system. In particular, one can not apply functions over one type to values of the other type and theorems about one type are not directly applicable to the other type. Moreover, while \(\inte\) is defined as a quotient type of \(\nat \times \nat\), from a semantic perspective, \(\nat\) should be a subtype of \(\inte\).

Before we can lift definitions or translate theorems from one type to another, we first need a framework to express the correspondence between two types. For this, Reynolds [ref] introduced relational prametricity. Intuitively, we can model a single type as a set and the fundamental idea of relational parametricity is that we can model the connection between a pair of types as a set of pairs, i.e.\ a relation. We call such relations transfer relations. Every type which we do not want to translate is related to itself by the identity relation. The relation \(\mathsf{T}_{\nat, \inte} :: \nat \Rightarrow \inte \Rightarrow \bool\) relates each non-negative integer with the corresponding natural number and the relation \(\mathsf{T}_\inte :: \nat \times \nat \Rightarrow \inte \Rightarrow \bool\) relates each integer with all its representations. Given these relation, we can state facts like \(\mathsf{T}_{\nat, \inte}\ 0_\nat\ 0_\inte\) and \(\mathsf{T}_\inte\ (n+3, n)\ 3\). We call such rules \(T\ x\ y\), where \(T\) is a transfer relations, transfer rules. Since type constructors like \((\Rightarrow)\) or \(\mathsf{set}\) and \(\mathsf{fset}\) map types to types, we express their connection by mappings from relations to relations. For instance, a function has type \(\alpha \Rightarrow \beta\) if it maps value of type \(\alpha\) to values of type \(\beta\) and analogously two functions are related by \(R \Rrightarrow S\) if they map inputs related by \(R\) to results related by \(S\).

Inspired by the work of Mitchell on representation independence [ref], the Transfer tool [ref] implements this idea to translate properties about one type to properties about another type. For example, when providing the right transfer rules, this tool can translate the proposition \(\forall m\ n :: \nat\ldotp 0 < m \longrightarrow 0 < m * n\) to \(\forall i\ j \in \{i :: \inte \mid 0 \leq i\}\ldotp 0 < i \longrightarrow 0 < i * j\).

While the Transfer tool translates propositions, the Lifting tool lifts definitions from one type to another. For instance, this tool can lift the addition operator \(\mathsf{add_{\inte rep}}\ (m_1, n_1)\ (m_2, n_2) \coloneqq (m_1 + m_2, n_1 + n_2)\) on the representation level of integers to the type \(\inte \Rightarrow \inte \Rightarrow \inte\) after the user provided a respectfulness theorem which states that \(\mathsf{add_{\inte rep}}\) respects the equivalence classes of the relation used in the definition of \(\inte\) as a quotient of \(\nat \times \nat\).

% fix transition

While it is easy to work with HOL types since in this type system, every term has a unique type and types are automatically checked and inferred, often a more expressive and flexible type system is desirable. For instance, with HOL types, if one wants to encode a property like finiteness of sets in a type, one has to define a new type. Since, from the perspective of the type system, these type are not related, operations defined on one type can not be used for the other type. Similarly, the sematic connection between the types \(\nat\), \(\inte\) and \(\rat\) can not be expressed in the HOL type system and hence theorems proven about \(\nat\) are not directly applicable to the set of non-negative integers. To some extend, one can use the Transfer- and Lifting-Tool to translate theorems and lift definitions from one type to another. This can, however become quiet cumbersome. Especially when one takes the approach common in mathematics to encode a variety of algebraic structures, datatypes and even functions as sets, the HOL type system is of little use to prevent errors otherwise detected by a type checker.

Soft types allow the user to encode arbitrary properties in a type. In this type system, a type is represented by a predicate over an HOL type. If a term \(x\) has the soft type \(\tau\), we write \(x : \tau\). The most primitive way to construct a soft type is using the function \(\mathsf{type}\) which takes a predicate as input and return the soft type containing exactly the values satisfying that predicate. Formally, \(x : \mathsf{type}\ P\) iff \(P\ x\). For instance, the soft type \(\mathsf{type}\ (\lambda i :: \inte\ldotp 0 \leq i)\) encodes the natural numbers within the HOL type \(\inte\). To further refine a given soft type \(T\) with a predicate \(P\), one can use the operator \((\adj)\) where \(x : P \adj T\) iff \(P\ x \land x : T\). Since one often wants do define a soft type which inhabits all elements of a given set, we use the notation \([A]\) as an abbreviation for \(\mathsf{type}\ (\lambda x\ldotp x \in A)\).

The last kind of soft type we use in this paper are dependent function types. A function \(f\) has the soft type \((x : \sigma) \rightarrow \tau\) iff \(\forall x\ldotp x : \sigma \longrightarrow f\ x : \tau\) where \(\tau\) may contain the variable \(x\). If the co-domain type does not depend on the argument, we use the simpler notation \(f : \sigma \rightarrow \tau\).

\begin{itemize}
	\item limitation of Lifting tool (supports too restrictive class of transfer relations for lifting between soft types)
	\item contribution
...
We then continue to explain how we can generalise the notion of transfer relations
...
may inhibit the same Pure-type $\mathtt{set}$.
...
When lifting terms in the context of data refinement,
\kevin{add citations to \url{https://www21.in.tum.de/~lammich/autoref/Refine/document.pdf} and \url{https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.800.3800&rep=rep1&type=pdf\#page=111}}
...
More specifically, consider the case of a transfer relation \(T :: \alpha \Rightarrow \beta \Rightarrow \bool\).
Since $T$ is right-total,
the statements \(\incodom\ T\ y\) and \(y :: \beta\) are equivalent.
\kevin{why is this important? When we find the lifting triple, we always have $\incodom\ T\ (abs\ x)$; I think you are basically trying to convey a problem about the soft type of $\Rrightarrow$, but it is very unclear in the current form. Let's discuss on Wednesday and update it together.}.
...
This can be used to select suitable transfer relations given a Pure-type.
...
If $T$ is a lifting relation, however, it might not be right-total.
Note that, theoretically, we could always choose a soft type for \(T\)
such that \(T\) is right-total with respect to that type,
namely \(T : \alpha \rightarrow \mathsf{type}\ (\incodom\ T) \rightarrow \bool\),
but this is in general not the type one would like to use and specify for $y$.

To see why the soft type \(\mathsf{type}\ (\incodom\ T)\) can inhabit both
more and fewer values than one would expect,
first consider the \liftrel\ \(T \coloneqq T_1 \Rrightarrow T_2\) where \(T_1\) is not right-unique.
...
However, if \(T_1\) has the soft type \(\alpha_1 \rightarrow \beta_1\rightarrow \bool\) and is right-total and \(T_2\) has the type \(\alpha_2 \rightarrow \beta_2\rightarrow\bool\),
we cold still prove the implication \((\incodom\ (T_1 \Rrightarrow T_2)\ f) \longrightarrow f : \beta_1 \rightarrow \beta_2\).

To see why this implication does not hold in general,
...
\kevin{again, this should be \(\eqabs\ T_3\ (h\ g_1)\ (f\ g_2)\) for some $h$}
As before, \(\eqabs\ T_3\ (f\ g_1)\ (f\ g_2)\) is a stronger conclusion
than \(f\ g_i : \beta_3\) for \(i \in \{1, 2\}\).
But now, also the premise is stronger:
from \(\incodom\ T\ f\),
we can only say something about \(f\ g\) if \(\incodom\ (T_1 \Rrightarrow T_2)\ g\), which
% as discussed above,
is only true for a subset of all functions with soft type \(\beta_1 \rightarrow \beta_2\).
...
the Pure-types or soft types of the representation and abstract term.
Instead, our lifting algorithm takes a \liftrel\ of the target type as input.
If one does not explicitly want to specify the \liftrel\ but only the type of the input term and the target type,
% in some type system like Pure-types or soft type,
an additional phase preceeding the algorithm can be implemented
which computes a \liftrel from the given types.
\kevin{this seems to contradict the sentence preceeding it}
...
\section{Application to Set Extensions}
From the examples in the sections above, we have seen, how we can construct new algebraic structures from already existing ones (eg. the construction of \(\rat\) from \(\inte\)) and how we can restrict a given type to encode further properties in a new type (eg. refining \(\mathsf{set}\) to \(\mathsf{fset}\)). We have also seen, that we can not directly apply operations defined over one type to values of another type. For instance we can not apply the union operator for \(\alpha\ \mathsf{set}\) to values of type \(\alpha\ \mathsf{fset}\) and multiply integers with the multiplication operator we defined over \(\rat\). Instead, we have to lift these operations. Because of this incomparability of \(\inte\) and \(\rat\) or \(\mathsf{set}\) and \(\mathsf{fset}\), we also were not able to directly reuse theorems about one type for the other type, but we had to transfer them.

Most mathematicians circumvent this problem by using set theoretic constructions instead of types. In this framework, everything is a set and \(\nat \subset \inte \subset \rat\) and \(\alpha\ \mathsf{fset}\) is a subsets of \(\alpha\ \mathsf{set}\).

However when, for instance, using standard constructions of \(\inte\) like a quotient of \(\nat \times \nat\) or a sum of \(\nat\) and \(\nat_+\), one faces the problem that \(\nat\) is not a subset of \(\inte\). Set extensions \juli{I haven't found a reference for this.} solve this problem by constructing a new set as the union of \(\nat\) and all elements in the representation set of \(\inte\) which do not represent a value already included in \(\nat\). To formalize and generalize this, we first need a definition of \emph{bijections} between two sets.
...
To obtain such a bijection, we define \(\mathsf{ext}\ A\ B\ f \coloneqq (B', rep, abs)\) where
\begin{align}
...
\end{align}
where \( (x, y) \) is the pair containing \(x\) and \(y\) and \(\snd\ (x, y) = y\). \juli{Again, I only found the Isabelle theory as a reference.}

That these definitions satisfy the properties stated above was proven in Isabelle/HOL. It is, however, not convenient to directly define operations on the set \(B'\) since such definitions would require case distinctions between elements which are contained in the extended set and those which are not. Instead, an operation on \(B'\) is usually defined as a combination of the respective operation on \(B\) and the functions \(abs\) and \(rep\) which translate between \(B\) and \(B'\).

\begin{example}
	Let us define the representation set of integers as \(\inte_{\rep} \coloneqq \nat \uplus \nat_+\) where \(A \uplus B\) is the disjoint union of the sets \(A\) and \(B\) with the injections \(\mathsf{in_l}\) and \(\mathsf{in_r}\) from \(A\) and \(B\) into \(A \uplus B\), reps, and \(\nat_+ \coloneqq \nat \setminus \{0\}\). Now we can construct the set extension \((\inte, \rep_\inte, \abs_\inte) \coloneqq \mathsf{ext}\ \nat\ \inte_{\rep}\ \mathsf{in_l} \). Then there is a bijection between \(\inte\) and \(\inte_{\rep}\) witnessed by \(\rep_\inte\) and \(\abs_\inte\) and we have \(\nat \subset \inte\).
	
	Given a multiplication function \(\mathsf{mul_{\inte rep}} : [\inte_{\rep}] \rightarrow [\inte_{\rep}] \rightarrow [\inte_{\rep}]\) on the representation level, we can lift this operator to \(\inte\) by \(\mathsf{mul_\inte}\ i\ j \coloneqq \abs_\inte\ (\mathsf{mul_{\inte rep}}\ (\rep_\inte\ i)\ (\rep_\inte\ j))\).
\end{example}

While we can not apply the original Lifting-Tool for the lifting in the above example, we can use our generalized approach to lifting. For this, we need a lifting triple to lift terms from \(\inte_{\rep}\) to \(\inte\). More generally, we can obtain a lifting triple from any bijection.
...
\begin{example}[Lifting triple for \(\inte\)]
	The bijection theorem of this set extension from the example above is \(\bijection\ \inte_{\rep}\ \inte\ \abs_\inte\ \rep_\inte\). From this, we get the lifting triple \(\langle \relint, \abs_\inte, \rep_\inte \rangle\) where \(\relint \coloneqq \birel\ \inte_{\rep}\ \abs_\inte\). In particular, \(\relint\ (\mathsf{in_l}\ n)\ n\) for any \(n \in \nat\).
\end{example}

From \cref{lem:lift-trip-bij}, we get the following corollaries for two common scenarios:
...
\begin{corollary}[Identity lifting triple for an HOL type]
	For any HOL type \(\alpha\), there is the lifting triple \(\langle \eq\ \univ_\alpha, \id, \id \rangle\) where \(\univ_\alpha\ \coloneqq \{x \mid x :: \alpha\}\).
\end{corollary}

To build the \liftrel to lift \(\mathsf{mul_{\inte rep}} : [\inte_{\rep}] \rightarrow [\inte_{\rep}] \rightarrow [\inte_{\rep}]\) to \(\mathsf{mul_{\inte}} : [\inte] \rightarrow [\inte] \rightarrow [\inte]\) or the divisibility predicate \(\mathsf{divides_{\inte rep}} : [\inte_{\rep}] \rightarrow [\inte_{\rep}] \rightarrow \bool\) to \(\mathsf{divides_{\inte}} : [\inte] \rightarrow [\inte] \rightarrow \bool\), we can use the previously introduced function relator \((\Rrightarrow)\) together with the \liftrels \(\mathsf{Rel_\inte}\) and \(\eq\ \univ_{\mathsf{bool}}\). However, in some cases this function relator is not sufficient. As an example, consider an operator for integer division \(\mathsf{div_\inte}\). We allow this function to be undefined or return an arbitrary value if its firs argument is not divisible by its second argument. Hence, \(\mathsf{div_\inte}\) has the dependent type \((i : [\inte]) \rightarrow (\lambda j\ldotp \mathsf{divides_\inte}\ j\ i) \boldsymbol{\cdot} [\inte] \rightarrow [\inte]\). Analogously, we also need a dependent function relator to relate the representation and abstraction version of this operator, since these must only produce related results for a subset of pairs of integers which is dependent on the first argument. We can define this dependent function relator analogous to the dependent function type constructor, intuitively substituting types by relations:
...
We extend the syntax introduced for the non-dependent function relator as follows:
\begin{equation*}
[x\ y \related T_1] \Rrightarrow T_2 \coloneqq \relfund\ T_1\ (\lambda x'\ y'\ldotp T_2[x \backslash x', y \backslash y'])
\end{equation*}

To construct a lifting triple with this relator, we also have to generalize \(\mapfun\) to \(\mapfund\) st. the function which maps the output of the mapped function may depend on the input of the mapped function.
...
\begin{lemma}[Lifting triple with dependent function relator and map function]
Given a lifting triple
\begin{equation*}
	\langle T_1, abs_1, rep_1 \rangle
\end{equation*}
and a parametrized lifting triple
\begin{equation*}
	\forall x\ y \ldotp T_1\ x\ y \longrightarrow \langle T_2\ x\ y, abs_2\ y\ x, rep_2\ x\ y\rangle
\end{equation*}
if \(T_2\ x\ y\) does not depend on which representatives \(x\) and \(y\) are chosen from any given equivalence class of \(\eqrep\ T_1\) and \(\eqabs\ T_1\), reps., and \(abs_2\) and \(rep_2\) each behave equivalently for equivalent arguments, formally
\begin{enumerate}
	...
	\null\quad\(((\eqabs\ (T_2\ x\ y)) \Rrightarrow (\eqrep\ (T_2\ x\ y)))\ (rep_2\ x\ y)\ (rep_2\ x'\ y')\) and
	\item \(\forall x\ x'\ y\ y' \ldotp T_1\ x\ y \land \eqrep\ T_1\ x\ x' \land \eqabs\ T_1\ y\ y' \longrightarrow \) \\
	\null\quad\(((\eqrep\ (T_2\ x\ y)) \Rrightarrow (\eqabs\ (T_2\ x\ y)))\ (abs_2\ y\ x)\ (abs_2\ y'\ x')\),
\end{enumerate}
then
\begin{equation*}
...
If the abstraction and representation functions of the lifting triple for the co-domain are not dependent on the arguments, we can use the following corollary:

\begin{corollary}[Lifting triple with dependent function relator]\label{cor:lift-trip-dep-fun}
	Assuming \(\langle T_1, abs_1, rep_1 \rangle\) and \(\forall x\ y \ldotp T_1\ x\ y \longrightarrow \langle T_2\ x\ y, abs_2, rep_2\rangle\) we have \(\langle \relfund\ T_1\ T_2, \mapfun\ rep_1\ abs_2, \mapfun\ abs_1\ rep_2 \rangle\) if \(\forall x\ x'\ y\ y' \ldotp T_1\ x\ y \land \eqrep\ T_1\ x\ x' \land \eqabs\ T_1\ y\ y' \longrightarrow T_2\ x\ y = T_2\ x'\ y'\).
\end{corollary}

Now, we can apply this corollary to the problem of lifting the integer division operator.

\begin{example}[Lifting integer division] The \liftrel which relates the integer division operators on \(\inte_{\rep}\) and \(\inte\) is
\begin{equation*}
	\mathsf{T_{div}} \coloneqq [\_\ i \related \relint] \Rrightarrow \reland\ \relint\ (\appabs\ (\mathsf{divides_\inte}\ i)) \Rrightarrow \relint
\end{equation*}
where
\begin{align*}
	\appabs\ f &\coloneqq \maprel\ f\ \pi_\abs \\
	\mathsf{\pi_{abs}}\ \_\ y &\coloneqq y \\
	\reland\ R\ S &\coloneqq \aprel\ (\maprel\ (\land)\ R)\ S \\
	\maprel\ f\ R\ x\ y &\coloneqq f\ (R\ x\ y) \\
	\aprel\ R\ S\ x\ y &\coloneqq R\ x\ y\ (S\ x\ y).
\end{align*}
To show \(\mathsf{T_{div}}\ \mathsf{div_{\inte rep}}\ (\mapfun\ \rep_\inte\ (\mapfun\ \abs_\inte\ \rep_\inte))\ \mathsf{div_{\inte rep}}\), it remains to prove
\begin{enumerate}
	\item \(\forall i\ j\ldotp \relint\ i\ j \longrightarrow \langle \reland\ \relint\ (\appabs\ (\mathsf{divides_\inte}\ j)) \Rrightarrow \relint, \mapfun\ \rep_\inte\ \abs_\inte, \mapfun\ \abs_\inte\ \rep_\inte \rangle\) and
	\item \(\forall i\ i'\ j\ j'\ldotp \relint\ i\ j \land \eqrep\ \relint\ i\ i' \land \eqabs\ \relint\ j\ j' \longrightarrow\)\\
	\null\quad\( (\reland\ \relint\ (\appabs\ (\mathsf{divides_\inte}\ j)) \Rrightarrow \relint)\ i\ j =(\reland\ \relint\ (\appabs\ (\mathsf{divides_\inte}\ j)) \Rrightarrow \relint)\ i'\ j'\).
\end{enumerate}
\end{example}
While the proof obligations resulting from the side-conditions of \cref{cor:lift-trip-dep-fun} seem complicated, it should be possible to automate them in most practical cases.

\section{Conclusion}
In this paper we proposed a generalization of the approach to lifting used by the Lifting-Tool. While the Lifting-Tool is limited to right-total and -unique transfer relations, our approach supports a larger class of relations, which we called \liftrels. This generalization is, in particular, necessary to lift terms between soft types. Since we use \liftrels instead of right-total and -unique transfer relations, we also had to generalize quotient predicates \cite{huffman2013lifting} to lifting triples which subsume the former. While quotient predicates are closed under composition but are not symmetric, lifting triples are not closed under composition but are symmetric. However, we proved a restricted composition theorem for lifting triples, which, we argued, is sufficiently general for the application of lifting. Moreover, we demonstrated, how our generalized approach can be applied to lift terms between soft types in a set theoretic framework. In particular, we introduced a dependent function relator which allowed us to lift partial functions, which, in general, is not possible with the Lifting-Tool.

While we presented the theoretical foundation for a new approach to lifting, it remains further work to implement our algorithm in a tool like the Lifting-Tool or a modification of that tool. Since the composition of lifting relations and triples as well as the construction of lifting triples involving the dependent function relator require proofs of side-condition, it is worth investigating how these proof obligations can be either discharged fully automatically or simplified to goal more readable and convenient to the user. It also remains an open question, whether there are practical applications for lifting for which the class of \liftrels is not general enough.
