...
In \cref{sec-prelims}, we build the parametrized transfer relation \(\mathsf{T_{fset p}}\) by composing the relator \(\mathsf{rel_{set}}\) with a non-parametrized transfer relation \(\mathsf{T_{fset}}\). While being right-total and right-unique is preserved by relational composition, this is not the case for the Z-property. We can, however, prove a restricted composition theorem for \liftrels:
...
Before I explain, what implications the generalization from quotient predicates to lifting triples has for the lifting algorithm, I first demonstrate, how the scheme described in \cref{sec-prelims} is implemented by the Lifting-Tool. For this, let us lift a term \(f\) of type \((\inte \times \inte)\ \mathsf{set} \Rightarrow \nat \Rightarrow (\inte \times \inte)\ \mathsf{set}\) to a term \(?t\) with type \(\rat\ \mathsf{fset} \Rightarrow \nat \Rightarrow \rat\ \mathsf{fset}\). With \cref{eq:lifting-thm}, we can instantiate \(?t\) as \(?abs\ f\) given a quotient predicate \(\langle ?R, ?abs, ?rep, ?T \rangle\) and a proof of \(?R\ f\ f\). Hence, the main part of the lifting algorithm is concerned with instantiating and proving the quotient predicate \(\langle ?R, ?abs, ?rep, ?T \rangle\). For this, the Lifting-Tool compares the type of \(f\) with the target type \(\rat\ \mathsf{fset} \Rightarrow \nat \Rightarrow \rat\ \mathsf{fset}\).

Since the outer-most type constructor is \((\Rightarrow)\) in both types, the tool applies \cref{eq:fun-quot-pred} to reduce the problem in two sub-goal: Proving a quotient predicates to lift the domain from \((\inte \times \inte)\ \mathsf{set}\) to \(\rat\ \mathsf{fset}\) and to lift the co-domain from \(\nat \Rightarrow (\inte \times \inte)\ \mathsf{set}\) to \(\nat \Rightarrow \rat\ \mathsf{fset}\). In the first goal, the outer-most type constructor changes from \(\mathsf{set}\) to \(\mathsf{fset}\). Hence, the Lifting-Tool uses \cref{eq:quotient-comp} to split the lifting from \((\inte \times \inte)\ \mathsf{set}\) to \(\rat\ \mathsf{fset}\) in two steps: First from \((\inte \times \inte)\ \mathsf{set}\) to \(\rat\ \mathsf{set}\) and then from \(\rat\ \mathsf{set}\) to \(\rat\ \mathsf{fset}\). For the first part, \cref{eq:set-rel} is applied. This leaves the goal \(\langle \mathsf{R_\rat}, \abs_\rat, \rep_\rat, \mathsf{T_\rat} \rangle\) which was registered to the Lifting-Tool when defining the type \(\rat\). Strictly speaking, the Lifting-Tool always first applies the composition theorem when the outer-most type constructor changes, here from \(\inte \times \inte\) to \(\rat\). However, when there are no type arguments, the first premise is discharged by the identity quotient predicate \(\langle (=), \id, \id, (=) \rangle\), which forms the base case of the algorithm.

For the second premise of the composition theorem, we need a quotient predicate to lift from \(\rat\ \mathsf{set}\) to \(\rat\ \mathsf{fset}\). For this, we can use the quotient predicate \(\langle (=), \abs_\mathsf{fset}, \rep_\mathsf{fset}, \mathsf{T_{fset}} \rangle\) mentioned in \cref{sec-prelims}.

The lifting from \(\nat \Rightarrow (\inte \times \inte)\ \mathsf{set}\) to \(\nat \Rightarrow \rat\ \mathsf{fset}\) is again reduced to one goal for the domain and one for the co-domain. For the domain, the types on representation and abstraction level are identical and thus the Lifting-Tool applies the identity quotient predicate \(\langle (=), \id, \id, (=) \rangle\). Now, we are left with the goal for the co-domains \((\inte \times \inte)\ \mathsf{set}\) and \(\rat\ \mathsf{fset}\), which we already solved above.

Putting it all together, we got the abstraction function
\begin{equation*}
...
More specifically, consider the case of a right-total transfer relation \(T :: \alpha \Rightarrow \beta \Rightarrow \bool\).
For such a relation, the statements \(\incodom\ T\ y\) and \(y :: \beta\) are equivalent.
This can be used to select a transfer relation given an HOL type.
...
and we could always choose a soft type for \(T\) such that \(T\) is right-total with respect to that type.
In particular, \(T : \alpha \rightarrow \mathsf{type}\ (\incodom\ T) \rightarrow \bool\) is right-total by definition, but \(\mathsf{type}\ (\incodom\ T)\) is, in general, not the type one would like to use for $y$.

To see why the soft type \(\mathsf{type}\ (\incodom\ T)\) can be inhabit both, more and fewer values then one would expect, first consider the \liftrel\ \(T_1 \Rrightarrow T_2\) where \(T_1\) is not right-unique.
...
In theory, we could encode this respectfulness property in a soft type,
but this would result in types that are unsuitable for practical use.
However, if \(T_1\) has the soft type \(\alpha_1 \rightarrow \beta_1\) and is right-total and \(T_2\) has the type \(\alpha_2 \rightarrow \beta_2\),
we cold still prove the implication \(\incodom\ (T_1 \Rrightarrow T_2)\ f \longrightarrow f : \beta_1 \rightarrow \beta_2\).

To see why this implication is not valid for arbitrary \liftrels \(T_1\) and \(T_2\),
...
Consider the \liftrel\ \(T = (T_1 \Rrightarrow T_2) \Rrightarrow T_3\) where \(T_1 : \alpha_1 \rightarrow \beta_1\rightarrow \bool\), \(T_2 : \alpha_2 \rightarrow \beta_2 \rightarrow \bool\), \(T_3 : \alpha_3 \rightarrow \beta_3 \rightarrow \bool\), and  \(T_1\) is non-right-unique.
Then \(\incodom\ T\ f\) means that for any two functions \(g_1, g_2\),
if \(\eqabs\ (T_1 \Rrightarrow T_2)\ g_1\ g_2\) then \(\eqabs\ T_3\ (f\ g_1)\ (f\ g_2)\).
As before, \(\eqabs\ T_3\ (f\ g_1)\ (f\ g_2)\) is a stronger conclusion then \(f\ g_i : \beta_3\) for \(i \in \{1, 2\}\).
But now, also the premise is stronger.
In particular, from \(\incodom\ T\ f\),
we can only say something about \(f\ g\) if \(\incodom\ (T_1 \Rrightarrow T_2)\ g\), which
% as discussed above,
is only true for a subset of all functions with soft type \(\alpha_1 \rightarrow \alpha_2\).
...
the HOL types or soft types of the representation and abstract term.
Instead, our lifting algorithm takes a \liftrel\ instead of the target type as input.
If one does not want to explicitly specify the \liftrel\ but only the type of the input terms and the target types in some type system like HOL types or soft type,
an additional phase preceeding the algorithm can be implemented,
which computes a \liftrel from the given types.

\paragraph{Compositionality}
As we have seen in the example at the start of this section, to lift a term from the type \(\kappa\ \overline{\alpha}\) to \(\kappa'\ \overline{\alpha'}\), the lifting algorithm applies the composition theorem \cref{eq:quotient-comp}. Specifically, it composes the relator for \(\kappa\) applied to the transfer relations for the type arguments, there \(\mathsf{rel_{set}}\ \mathsf{T_\rat}\), with the non parametric transfer relation from \(\kappa\ \overline{\alpha'}\) to \(\kappa'\ \overline{\alpha'}\), there \(\mathsf{T_{fset}}\).
...
which can then automatically be discharged by the soft-type-checker. If the algorithm terminates without errors, it is guaranteed that it generates a valid lifting triple, since it only applies rules verified by Isabelle.
...
As explained above, the algorithm of the Lifting-Tool is, in essence, a recursion over the structure of the type of the representation value and the target type. Hence, the correctness proof provided in \cite{huffman2013lifting} is a proof by induction over these types.
Since the new algorithm is not guided by types but the structure of transfer relations, we need a more flexible approach to prove termination.
As before, each rule which the algorithm applies has only finitely many premises and in particular finitely many lifting triple premises.
Thus, if we want to guarantee termination, we may attempt to find some well-founded order on \liftrels and only accept rules respecting it, i.e. rules \(\overline{P} \longrightarrow Q\) for some lifting triple \(Q\) st. the \liftrel of each lifting triple premise in \(\overline{P}\) is less then the \liftrel of \(Q\) wrt. the well-founded order.
In practice, I suggest a sub-term ordering on the \liftrels, since this is in close connection to the sub-term ordering on types used for the Lifting-Tool.
