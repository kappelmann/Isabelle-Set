...
next
  ...
  have "T1 x e"
    ...
    using b(1) e(1) .
  moreover have "Eq_rep2 e c"
    using e(2) d(2) assms(2) unfolding lifting_unfold
    by metis
  ultimately show "rel_comp' (rel_inv T1) (rel_comp' Eq_rep2 T1) x z"
    unfolding rel_comp'_def
    using b a(2) d c(2)
    by blast
next
...
lemma
  assumes
    ...
    "rel_comp' T Eq_rep = T"           (* new because T\<^sup>-\<^sup>1 \<circ> T may not be transitive if T is not right-unique *)
    "\<And>x y. T x y \<Longrightarrow> Eq_abs (abs x) y" (* 2. condition with equality replaced by Eq_abs *)
    "\<And>x y. T x y \<Longrightarrow> Eq_rep x (rep y)" (* *)
  shows "lifting Eq_rep Eq_abs T abs rep"
proof (rule liftingI)
  fix x y
  assume "Eq_rep x y"
...
next
  ...
  assume prem: "Eq_abs y y"
  show "T (rep y) y"
    using prem assms(1, 2, 3, 5)
    using rel_comp'_def rel_inv_def
    by metis
qed
...
lemma
  assumes
    ...
    "Eq_abs = rel_comp' T (rel_inv T)" (* new because there was no Eq_abs before *)
    "rel_comp' T Eq_rep = T"           (* new because T\<^sup>-\<^sup>1 \<circ> T may not be transitive if T is not right-unique *)
    "\<And>x. Eq_rep x x \<Longrightarrow> T x (abs x)" (* *)
    "\<And>y. Eq_abs y y \<Longrightarrow> T (rep y) y"   (* 3. condition weakened because T is not right-total anymore *)
  shows "lifting Eq_rep Eq_abs T abs rep"
proof (rule liftingI)
  fix x y
...
lemma
  "lifting Eq_rep Eq_abs T abs rep \<Longrightarrow> (\<And>x. Eq_rep x x \<Longrightarrow> T x (abs x))"
  unfolding lifting_unfold by metis

lemma
  "lifting Eq_rep Eq_abs T abs rep \<Longrightarrow> (\<And>x y. T x y \<Longrightarrow> Eq_rep x (rep y))"
  unfolding lifting_unfold by metis
