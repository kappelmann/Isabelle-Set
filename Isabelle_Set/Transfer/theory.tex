...
\begin{lemma}[Parameterized lifting triple for functions]
...
\begin{example}[Lifting triple for functions over \(\inte\)]
	...
	\begin{align*}
		\langle \relfun\ \relint\ (\relfun\ \relint\ \relint),\\
	...
	Unfolding the definition of \(\mapfun\), we get \(\mathsf{mul_\inte}\ i\ j = \abs_\inte\ (\mathsf{mul_{\inte_{\rep}}}\ (\rep_\inte\ i)\ (\rep_\inte\ j))\), as expected.
	...
	\begin{equation*}
		\relfun\ \relint\ (\relfun\ \relint\ (\eq\ \univ_{\mathsf{bool}}))
	\end{equation*}
	and
	\begin{equation*}
		\relfun\ \relint\ (\relfun\ (\eq\ \nat)\ \relint)
	\end{equation*}
	resp.
\end{example}
