...
\end{definition}
	...
	\begin{example}
		...
		Given a transfer triple
		\begin{equation*}
			\langle T_1, abs_1, rep_1 \rangle
		\end{equation*}
		and a parameterized transfer triple
		\begin{equation*}
			\forall x\ y \ldotp T_1\ x\ y \longrightarrow \langle T_2\ x\ y, abs_2\ x, rep_2\ y\rangle
		\end{equation*}
		...
		\begin{align*}
			\forall x\ x'\ y\ y' \ldotp T_1\ x\ y \land \eqrep\ T_1\ x\ x' \land \eqabs\ T_1\ y\ y' \longrightarrow T2\ x\ y = T2\ x'\ y' \\
			\forall x\ y\ y' \ldotp T_1\ x\ y \land \eqabs\ T_1\ y\ y' \longrightarrow (\relfun\ (\eqabs\ (T_2\ x\ y))\ (\eqrep\ (T_2\ x\ y)))\ (rep_2\ y)\ (rep_2\ y') \\
			\forall x\ x'\ y \ldotp T_1\ x\ y \land \eqrep\ T_1\ x\ x' \longrightarrow (\relfun (\eqrep\ (T_2\ x\ y))\ (\eqabs\ (T_2\ x\ y)))\ (abs_2\ y)\ (abs_2\ y')
		\end{align*}
		then
		\begin{equation*}
		...
		\begin{align*}
			...
			\mapfund\ f\ g\ h\ x &:= g\ (f\ x)\ (h\ (f\ x))
		\end{align*}
	\end{example}
	
	\begin{theorem}[Composition of lifting triples]
		Given two lifting triples \(\langle T_1, abs_{T_1}, rep_{T_1} \rangle \) and \(\langle T_2, abs_{T_2}, rep_{T_2} \rangle \), they can be composed to new lifting triple if \(\eqabs\ T_1\) commutes with \( \eqrep\ T_2 \) under relation composition. Formally
		\begin{equation}
			\begin{split}
	...
	\begin{lemma}[Dual lifting triple]
		\begin{equation}
