...
theory Lists
  imports
    Isabelle_Set.TSLeast_Fixpoints
...
lemma [type]: "A \<Ztypecolon> Subset (univ A)" by unfold_types auto
lemma [type]: "nil \<Ztypecolon> Element (univ A)"
  unfolding nil_def by unfold_types auto
lemma [type]:
  "cons \<Ztypecolon> Element (univ A) \<Rightarrow> Element (univ A) \<Rightarrow> Element (univ A)"
  unfolding cons_def by unfold_types auto
lemma "A \<Longrightarrow> (A \<Longrightarrow> B) \<Longrightarrow> B"
  by (rule Pure.meta_impE)

lemma list_op_Monop [type]: "list_op \<Ztypecolon> (A : Set) \<Rightarrow> Monop (univ A)"
  ...
  apply (intro Dep_funI)
  apply unfold_types
  apply (simp only: mem_of_eq)
  apply (drule subset_if_mem_powerset)
  apply (drule mono_subset_pair[THEN mono_wrt_relD, THEN Fun_Rel_relD, OF subset_self])
  apply (drule subsetD)
    apply assumption
  apply (tactic \<open>rotate_tac 2 1\<close>)
  apply (drule subsetD[OF mono_subset_pair[THEN mono_wrt_relD, THEN Fun_Rel_relD, OF _ subset_self], OF subset_univ])
  apply auto
  done

lemmas list_fold =
  fixpoint_lfp[OF Dep_funD[OF list_op_Monop AnyI],
    folded list_def, unfolded fixpoint_def]

lemma nil_type [type]: "nil \<Ztypecolon> Element (list A)"
  by (subst list_fold[symmetric]) (unfold_types, unfold list_op_def, auto)

lemma cons_type [type]: "cons \<Ztypecolon> Element A \<Rightarrow> Element (list A) \<Rightarrow> Element (list A)"
  by (subst (2) list_fold[symmetric]) (unfold_types, unfold list_op_def, auto)

lemma list_type [type]: "list \<Ztypecolon> (A : Set) \<Rightarrow> Subset (univ A)"
  unfolding list_def by discharge_types

lemma list_induct:
  assumes xs_type: "xs \<Ztypecolon> Element (list A)"
  assumes nil: "P nil"
  assumes cons:
    "\<And>x xs. x \<Ztypecolon> Element A \<Longrightarrow> xs \<Ztypecolon> Element (list A) \<Longrightarrow> P xs \<Longrightarrow> P (cons x xs)"
  shows "P xs"
proof (rule lfp_induct[OF Dep_funD[OF list_op_Monop AnyI], of xs A P,
...
lemma list_rec_op_Monop [type]:
  shows "list_rec_op :
    (A : Set) \<Rightarrow> Element (M nil) \<Rightarrow> ((x \<Ztypecolon> Element A) \<Rightarrow> (xs \<Ztypecolon> Element (list A)) \<Rightarrow>
...
axiomatization list_rec :: "set \<Rightarrow> (set \<Rightarrow> set \<Rightarrow> set \<Rightarrow> set) \<Rightarrow> set \<Rightarrow> set"
  ...
  and list_rec_cons: "x \<Ztypecolon> Element A \<Longrightarrow> xs \<Ztypecolon> Element (list A) \<Longrightarrow>
    list_rec n c (cons x xs) = c x xs (list_rec n c xs)"

lemma list_rec_type [type]:
  "list_rec \<Ztypecolon> X \<Rightarrow> (Element A \<Rightarrow> Element (list A) \<Rightarrow> X \<Rightarrow> X) \<Rightarrow> Element (list A) \<Rightarrow> X"
proof (intro Dep_funI)
  fix N C xs
  assume [type]: "N \<Ztypecolon> X"
    "C \<Ztypecolon> Element A \<Rightarrow> Element (list A) \<Rightarrow> X \<Rightarrow> X" "xs \<Ztypecolon> Element (list A)"
  show "list_rec N C xs \<Ztypecolon> X"
...
lemma append_type [type]:
  "append \<Ztypecolon> Element (list A) \<Rightarrow> Element (list A) \<Rightarrow> Element (list A)"
proof -
  have "(\<lambda>x _. cons x) \<Ztypecolon> Element A \<Rightarrow> Element (list A) \<Rightarrow> Element (list A)
...
lemma rev_type [type]: "rev \<Ztypecolon> Element (list A) \<Rightarrow> Element (list A)"
proof -
  have "(\<lambda>x _ acc. append acc (cons x nil)) \<Ztypecolon>
