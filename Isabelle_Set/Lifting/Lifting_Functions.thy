...
next
  ...
  have T_x_abs_x: "T x (abs x)"
    by (fact lifting_triple_rel_abs_if_in_dom[OF lift_trip, OF in_dom_T_x])
...
next
  ...
  have T_rep_y_y: "T (rep y) y"
    by (fact lifting_triple_rel_rep_if_in_codom[OF lift_trip, OF in_codom_T_y])
  have T_x_y: "T x y" and P_x_y: "P x y"
    by (rule rel_restE[OF rel_rest_x_y], assumption)+
...
lemma map_fun_eq: "map_fun f g h = g \<circ> h \<circ> f"
...
proof -
  ...
  have Eq_rel2: "T2 x y = T2 x (abs1 x)"
    using T2_resp rel_arg(1) Eq_rep_self_if_in_dom in_domI rel_arg(1) Eq_abs_sym
      lifting_triple_Eq_abs_abs_if_rel_x_y .
...
proof (rule lifting_tripleI)
  show "z_property T"
    using z_property_if_lifting_triple lift_trip .
next
  fix x
...
next
  ...
  proof (rule Dep_Fun_RelI)
    ...
    have 2: "Eq_abs T1 y (abs1 x)"
      using lifting_triple_Eq_abs_abs_if_rel lift_trip1 rel .
...
lemma lifting_triple_Dep_Fun_RelI':
  ...
  apply (rule lifting_triple_rel_abs_if_Eq_rep[OF lift_trip2])
   apply assumption
  apply (rule rel_if_sym_rel_if_partial_equivalence[OF partial_equivalence_Eq_rep_if_z_property,
    OF z_property_if_lifting_triple, OF lift_trip2])
  apply assumption+
  done
...
next
  ...
  { fix x z
    ...
    have 6: "T2 (f (rep1 z)) (abs2 (f (rep1 z)))"
      using lifting_triple_rel_abs_if_in_dom assms(2) in_dom_if_Eq_rep Eq_rep_sym 5 .
