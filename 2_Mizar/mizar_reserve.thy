...
fun do_var gen (v as (n, _)) lthy =
  ...
  | SOME D =>
    let
      ...
      val (_, lthy2) = if Variable.is_fixed lthy1 n then ([], lthy1) else Variable.add_fixes [n] lthy1;
    ...
    in
      ((assm :: flat assms), if gen then lthy3 else Context.proof_map (add_normalized_ty_data assm) lthy3)
    end
fun do_var_nofail gen (v as (n, _)) lthy =
  let
    fun justfix lthy = (if Variable.is_fixed lthy n then ([], lthy) else ([], snd (Variable.add_fixes [n] lthy)))
  in
  
  do_var gen v lthy handle Fail _ => justfix lthy
                         | TYPE _ => justfix lthy
  end
\<close>
...
fun do_lt tm lthya =
   case dest_Trueprop tm of
     (Const (@{const_name ty_membership}, _) $ Free (n, _) $ _) =>
       ...
       in
         (assm, Context.proof_map (add_normalized_ty_data assm) lthya3)
       end
   | _ =>
       let
         val ct = Thm.cterm_of lthya tm;
...
fun normalize_thm ctxt th =
  let
    ...
    val ss = put_simpset main_ss ctxt addsimps @{thms object_root ty_intersection} addsimprocs [@{simproc ex_simp}]
    val th3 = Simplifier.asm_full_simplify ss th2
              handle THM _ => th2
...
fun mdecl nameoption decl lt defn getth afterqed lthy =
 let
   ...
   val (ret, lthy2) = Specification.definition_cmd (SOME decl) [] [] (Binding.empty_atts, defn) false lthy1
 in
...
fun mdecl_cmd nameoption decl lt defn getth afterqed lthy =
 let
   ...
   val (ret, lthy2) = Specification.definition' (SOME decl) [] [] (Binding.empty_atts, defn) false lthy1
 in
...
val parser = (Parse_Spec.constdecl --
...
fun conj1_ex name (thm, lthy) =
  let
    val thm_ex = @{thm conjunct1} OF [thm]
    val lthy' = note_suffix name "_ex" thm_ex lthy
    val lthy'' = Local_Theory.declaration {syntax = false, pervasive = false}
      (fn phi => Named_Theorems.add_thm "mizar_ty.ex" (Morphism.thm phi thm_ex)) lthy'
  in
    (@{thm conjunct2} OF [thm], lthy'')
  end
*}*)
...
(*
fun conj1_ty_func_cluster name (thm, lthy) =
  (@{thm conjunct2} OF [thm], snd (Local_Theory.note ((Binding.make (name ^ "_ty_func_cluster", Position.none), @{attributes [simplified,rule_format,ty_func_cluster]}), [@{thm conjunct1} OF [thm]]) lthy))
*)
...
( type T = (thm * ((string -> thm * local_theory -> thm * local_theory) list *
     Token.src list)) Symtab.table;
  val empty = Symtab.empty;
  val extend = I;
...
setup {* fn thy => thy |>
add_miz_ident @{const_name "ClusterFuncNoFor"} @{thm ClusterFuncNoFor_property}
   [] @{attributes [simplified, rule_format, clus]} |>
add_miz_ident @{const_name "ClusterFuncFor"} @{thm ClusterFuncFor_property} []
  @{attributes [simplified, rule_format, clus]} |>
add_miz_ident @{const_name "ClusterExistence"} @{thm ClusterExistence_property} []
     @{attributes [simplified, rule_format, ex]} |>
add_miz_ident @{const_name "ClusterCondAttrsAttrsTy"} @{thm ClusterCondAttrsAttrsTy_property} []
     @{attributes [simplified, rule_format, clus]} |>
add_miz_ident @{const_name "ClusterCondAttrsTy"}  @{thm ClusterCondAttrsTy_property} []
   @{attributes [simplified, rule_format, clus]} |>

add_miz_ident @{const_name "redefine_func_means"}  @{thm redefine_func_means_property}
  [c1_note_attr "_uniq"  @{attributes [rule_format]}] [] |>
add_miz_ident @{const_name "redefine_func_assume_means"}  @{thm redefine_func_assume_means_property}
  [c1_note_attr "_uniq" @{attributes [rule_format]}] [] |>
add_miz_ident @{const_name "redefine_func_means_if1"}  @{thm redefine_func_means_if1_property}
  [c1_note_attr "_uniq" @{attributes [rule_format]}] [] |>
add_miz_ident @{const_name "redefine_func_means_if1o"}  @{thm redefine_func_means_if1o_property}
  [c1_note_attr "_uniq" @{attributes [rule_format]}] [] |>

add_miz_ident @{const_name "redefine_func_equals"}  @{thm redefine_func_equals_property}
  [] @{attributes [rule_format]} |>
add_miz_ident @{const_name "redefine_func_assume_equals"}  @{thm redefine_func_assume_equals_property}
  [] @{attributes [rule_format]} |>
add_miz_ident @{const_name "redefine_func_equals_if1"}  @{thm redefine_func_equals_if1_property}
  [] @{attributes [rule_format]} |>

add_miz_ident @{const_name "redefine_func_type"}   @{thm redefine_func_type_property} []
    @{attributes [simplified, rule_format, clus]} |>

add_miz_ident @{const_name "redefine_attr_means"}   @{thm redefine_attr_means_property}
      [c1_note_attr "I" @{attributes [rule_format]},
      c1_note_attr "E" @{attributes [rule_format]}] [] |>

add_miz_ident @{const_name "redefine_mode_means"}   @{thm redefine_mode_means_property}
     [c1_note_attr "I" @{attributes [rule_format]},
      c1_note_attr "E" @{attributes [rule_format]}] [] |>

add_miz_ident @{const_name "redefine_pred_means"}   @{thm redefine_pred_means_property}
     [c1_note_attr "I" @{attributes [rule_format]},
      c1_note_attr "E" @{attributes [rule_format]}] [] |>

add_miz_ident @{const_name "redefine_pred_means_if1"}   @{thm redefine_pred_means_if1_property}
     [c1_note_attr "I" @{attributes [rule_format]},
      c1_note_attr "E" @{attributes [rule_format]}] []
...
setup {* fn thy => thy |>
add_miz_ident @{const_name "func_means"} @{thm func_means_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if1"} @{thm func_means_if1_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if2"} @{thm func_means_if2_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if1o"} @{thm func_means_if1o_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if2o"} @{thm func_means_if2o_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if3"} @{thm func_means_if3_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_means_if4"} @{thm func_means_if4_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_assume_means"} @{thm func_assume_means_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_assume_means_if1"} @{thm func_assume_means_if1_property} [conj1_ty_func, conj1_note_suffix "_uniq"] [] |>
add_miz_ident @{const_name "func_equals"} @{thm func_equals_property} [conj1_ty_func] [] |>
add_miz_ident @{const_name "func_equals_if1"} @{thm func_equals_if1_property} [conj1_ty_func] [] |>
add_miz_ident @{const_name "func_equals_if2"} @{thm func_equals_if2_property} [conj1_ty_func] [] |>
add_miz_ident @{const_name "func_equals_if1o"} @{thm func_equals_if1o_property} [conj1_ty_func] [] |>
add_miz_ident @{const_name "func_equals_if2o"} @{thm func_equals_if2o_property} [conj1_ty_func] [] |>
add_miz_ident @{const_name "func_assume_equals"} @{thm func_assume_equals_property} [conj1_ty_func] [] |>

add_miz_ident @{const_name "pred_means"} @{thm pred_means_property} [conj1_note_intro "I", conj1_note_elim "E"] [] |>
add_miz_ident @{const_name "pred_means_if1"} @{thm pred_means_if1_property} [conj1_note_intro "I", conj1_note_elim "E"] [] |>
add_miz_ident @{const_name "pred_means_if2"} @{thm pred_means_if2_property} [conj1_note_intro "I", conj1_note_elim "E"] [] |>
add_miz_ident @{const_name "pred_means_if3"} @{thm pred_means_if3_property} [conj1_note_intro "I", conj1_note_elim "E"] [] |>
...
add_miz_ident @{const_name "mode_means"} @{thm mode_means_property}
         [conj1_ex, conj1_clus, conj1_note_intro "I", conj1_note_elim "E"] [] |>
add_miz_ident @{const_name "mode_assume_means"} @{thm mode_assume_means_property} 
      [conj1_ex, conj1_clus, conj1_note_intro "I", conj1_note_elim "E"] [] |>
add_miz_ident @{const_name "mode_means_if1"} @{thm mode_means_if1_property} 
     [conj1_ex, conj1_clus, conj1_note_intro "I", conj1_note_elim "E"] [] |>

add_miz_ident @{const_name "attr_means"} @{thm attr_means_property} [conj1_note_intro "I", conj1_note_elim "E", conj1_note_intro "nI"] [] |>
add_miz_ident @{const_name "attr_assume_means"} @{thm attr_assume_means_property} [conj1_note_intro "I", conj1_note_elim "E", conj1_note_intro "nI"] [] |>
add_miz_ident @{const_name "attr_means_if1"} @{thm attr_means_if1_property} [conj1_note_intro "I", conj1_note_elim "E"] []
*}
...
fun mdefafterqed retlthy fname thms prop lthy' =
 let
  val (thm, lthy) = case retlthy of SOME lt => (the_single (Variable.export lthy' lt [Assumption.export false lthy' lt (the_single thms)]), lt) | NONE => (the_single thms, lthy');
...
fun mdef (nameopt, ((decl, lt), defn)) lthy =
  let
    fun getth prop = fst (mdef_identify lthy prop)
  in
    mdecl nameopt decl lt defn getth mdefafterqed lthy
  end;
*}
ML {*
fun mdef_cmd (nameopt, ((decl, lt), defn)) lthy =
  let
    fun getth prop = fst (mdef_identify lthy prop)
  in
    mdecl_cmd nameopt decl lt defn getth mdefafterqed lthy
  end;
*}

ML {*
val parser = (Scan.option (Parse.binding --| @{keyword ":"}) -- (Parse_Spec.constdecl --
    (Scan.option ((@{keyword "mlet"} || @{keyword "assumes"}) |-- Parse.list1 Parse.prop)) --
    Parse.prop)) >> (fn (b,t) => mdef (Option.map Binding.name_of b, t));
Outer_Syntax.local_theory_to_proof @{command_keyword mdef} "Mizar functor" parser
*}

ML {*
fun do_then ofth _ (thm, lthy) =
  (ofth OF [thm], lthy)
*}
...
fun mtheorem_cmd scheme generalize name attr lets prop lthy =
  let
    ...
    val as_fs = map_filter (fn x => SOME ((dest_Free o fst o dest_typing) x) handle TERM _ => NONE) lets
    val c_fs = fold Term.add_frees lets (Term.add_frees prop [])
    val fs = subtract (=) as_fs c_fs
    val _ = if not scheme andalso filter_out (fn (n, ty) => ty = @{typ Set} orelse Variable.is_fixed lthy n) fs <> [] then
    ...
    val (assms1, lthy1) = fold_map (if scheme then do_var_nofail generalize else do_var generalize) setfs lthy
    val (_, lthy2) = fold_map (do_var_nofail generalize) nosetfs lthy1
                     handle _ => ([], lthy1)
    val (assms2, lthy3) = fold_map do_lt lets lthy2
    val lthy4 = if generalize then lthy3 else
      #3 (gen_typing_data' lthy3 (prop :: (map Thm.prop_of (flat assms1 @ assms2))))
    fun after_qed thms lthyn =
      ((*tracing (@{make_string} thms);*)
...
(*
ML {*
fun func3 ((bind, mixf), str) int lthy =
  let
    val bind2 = Binding.map_name (suffix "_raw") bind
    val ((_, (_, th)), lthy') =
      Specification.definition_cmd (SOME (bind, NONE, mixf)) [] [] ((bind2, []), str) int lthy
    val (doth, procs) = mdef_identify (Thm.prop_of th);
    val didth = doth OF [th];
    val (thm, lthy'') = fold (fn p => fn sf => p (Binding.name_of bind) sf) procs (didth, lthy')
    val th = normalize_thm lthy'' thm
    val lthy''' = snd (Local_Theory.note ((bind, []), [th]) lthy'')
  in lthy''' end
val _ =
  Outer_Syntax.local_theory' @{command_keyword func3} "Mizar noprf definition"
    (((Parse.binding -- Parse.opt_mixfix') -- Parse.prop) >> func3);
*}
*)
