...
text \<open>Setup for proof steps related to logic. Adapted from \<open>Auto2_HOL/HOL/Logic_Thms.thy\<close>.\<close>
...
setup \<open>add_forward_prfstep_cond @{thm HOL.not_sym}
  [with_filt (not_type_filter "s" boolT)]\<close>
...
setup \<open>add_rewrite_rule @{thm fst_pair_eq}\<close>
setup \<open>add_rewrite_rule @{thm snd_pair_eq}\<close>
setup \<open>add_forward_prfstep (equiv_forward_th @{thm pair_eq_iff})\<close>
setup \<open>Auto2_Setup.Normalizer.add_rewr_normalizer ("rewr_case", @{thm uncurry_def})\<close>
