...
begin
  definition "surjective_at_type (T :: 'b type) :: ('a \<Rightarrow> 'b) \<Rightarrow> bool \<equiv> surjective_at (type_pred T)"
end

lemma surjective_at_type_eq_surjective_at_pred [simp]:
  "(surjective_at (T :: 'b type) :: ('a \<Rightarrow> 'b) \<Rightarrow> bool) = surjective_at (type_pred T)"
...
lemma surjective_at_type_iff_surjective_at_pred [iff]:
  "surjective_at (T :: 'b type) (f :: 'a \<Rightarrow> 'b) \<longleftrightarrow> surjective_at (type_pred T) f"
  by simp
