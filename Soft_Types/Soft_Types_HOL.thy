...
ML \<open>
fun full_discharge_types_tac more_facts add_tms ctxt =
  let
    val type_tac =
      let val refine_tac = resolve_tac ctxt [@{thm Int_typeI}, @{thm adjI}]
      in
        (TRY o REPEAT o refine_tac) THEN' Derivation.discharge_type_tac ctxt more_facts add_tms
      end
  ...
  fun solver ctxt i =
    ...
    in
      ...
      THEN SOLVED' (SUBGOAL (fn (t, i) =>
        ...
        then full_discharge_types_tac (Simplifier.prems_of ctxt) [] ctxt i
        else no_tac)
      )) i
    end
in
