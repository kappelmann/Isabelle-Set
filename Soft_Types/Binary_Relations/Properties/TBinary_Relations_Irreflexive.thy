...
begin
  definition "irreflexive_on_type (T :: 'a type) :: ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> bool \<equiv>
    irreflexive_on (of_type T)"
end

lemma irreflexive_on_type_eq_irreflexive_on_pred [simp]:
  "(irreflexive_on (T :: 'a type) :: ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> bool) = irreflexive_on (of_type T)"
...
lemma irreflexive_on_type_iff_irreflexive_on_pred [iff]:
  "irreflexive_on (T :: 'a type) (R :: 'a \<Rightarrow> 'a \<Rightarrow> bool) \<longleftrightarrow> irreflexive_on (of_type T) R"
  by simp
