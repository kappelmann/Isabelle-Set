...
begin
  definition "transitive_on_type (T :: 'a type) :: ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> bool \<equiv>
    transitive_on (type_pred T)"
end

lemma transitive_on_type_eq_transitive_on_pred [simp]:
  "(transitive_on (T :: 'a type) :: ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> bool) = transitive_on (type_pred T)"
...
lemma transitive_on_type_iff_transitive_on_pred [iff]:
  "transitive_on (T :: 'a type) (R :: 'a \<Rightarrow> 'a \<Rightarrow> bool) \<longleftrightarrow> transitive_on (type_pred T) R"
  by simp
