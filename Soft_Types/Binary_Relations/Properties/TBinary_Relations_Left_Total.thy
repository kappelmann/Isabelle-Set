...
begin
  definition "left_total_on_type (T :: 'a type) :: ('a \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> bool \<equiv>
    left_total_on (of_type T)"
end

lemma left_total_on_type_eq_left_total_on_pred [simp]:
  "(left_total_on (T :: 'a type) :: ('a \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> bool) = left_total_on (of_type T)"
...
lemma left_total_on_type_iff_left_total_on_pred [iff]:
  "left_total_on (T :: 'a type) (R :: 'a \<Rightarrow> 'b \<Rightarrow> bool) \<longleftrightarrow> left_total_on (of_type T) R"
  by simp
